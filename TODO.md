...
## UX/UI 개선 (2026-07-01 분석)

분석 요약: 디자인 완성도(따뜻한 에디토리얼 톤·카드 그리드·접근성 기초)는 높으나 곳곳에 "데모 티"가 남아 실결제 서비스 신뢰도를 깎음. **데모 흔적 제거 + 하드코딩 콘텐츠 동적화**가 최우선 레버.

### 🔴 런칭 전 (신뢰도 직결)
- [ ] "데모" 문구 전수 제거 — `index.html` meta description(line 8 "…커뮤니티 플랫폼 데모"), 후기 카운트 "데모 후기 3개"(line 181). 결제창 DEMO 분기는 `live_` 키 전환 시 자동 해결됨.
- [ ] 하드코딩 히어로 추천 카드 동적화/최신화 — `index.html` line 72-76 "토요일 밤의 취향 살롱 · 6월 13일 · 성수 · 4자리"(지난 날짜 + 수동 좌석 라벨, 동적 정원 시스템과 모순). 실데이터 연결 또는 최신 모임 자동 표시.
- [ ] 홈 후기 밴드 더미 3개 처리 — `index.html` line 184-203(가짜 후기/익명 라벨). 실제 후기 연동 또는 제거.
- [ ] Unsplash 핫링크 이미지 교체 — 히어로·카드 다수가 `images.unsplash.com` 직링크(라이선스·성능·끊김 리스크) → 소유/라이선스 이미지로.
- [ ] 푸터 사업자정보 `[[ ]]` 값 채우기 — 위 🔴 높은 우선순위 항목과 동일 건.

### 🟡 UX 개선
- [ ] 주 CTA 문구 구체화 — "시작하기" → "모임 둘러보기/신청하기" 등. "시작하기"·"여는이 제안하기"가 같은 apply 모달을 여는지 점검.
- [ ] 섹션 구분 명확화 — 모임/원데이/소모임/오픈예정 차이를 라벨·설명으로 구분하거나 통합.
- [ ] 모바일 내비 점검 — 하단 탭 3개(모임/알림/이벤트)로 축소됨. "내 신청 이력"·결제 진입 CTA의 모바일 접근성 확인.
- [ ] 결제 신뢰 신호 추가 — 홈/카드/CTA 근처에 안전결제·토스·환불정책 표기로 전환율·안심도 보강.
- [ ] 스켈레톤 로더 — Supabase 카드 로드 중 빈 그리드 대신 표시(load-retry/에러 처리는 이미 있음).
- [ ] 검색 버튼 색(노랑) vs 그린 주 CTA — 색 언어 통일 검토.

### ♿ 접근성 보강
- [ ] 본문 바로가기(skip link) 추가.
- [ ] 대비 점검 — muted 텍스트(#6d6a62), 노랑 검색 버튼 텍스트 WCAG AA 충족 여부.
